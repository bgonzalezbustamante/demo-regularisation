...
```{r ols1, warning = FALSE, message = FALSE}
...
The predictions are used to compare the test set and the Root Mean Square Error (RMSE) measure. The lowest values are better, however, it could compromise parsimonious.

```{r ols2, warning = FALSE, message = FALSE}
...
K-fold cross-validation implies a random division of the data set into $k$-$h$ folder, generally $k = 10$. The first fold is considered a test set for validation, and the models are fitted in the remaining ones. 

```{r kfold, warning = FALSE, message = FALSE, results = 'asis'}
...
LOOCV uses a simple observation of the dataset as a test set, while the remaining cases are considered a training set. Then, the model is fitted on the training set, and the predictions are used to compare with the excluded observation. 

```{r loocv, warning = FALSE, message = FALSE, results = 'asis'}
...
Ridge regression is similar to OLS, but it includes a shrinkage penalty *lambda* in the equation, which is small when *beta* coefficients are near to zero.

```{r ridge, warning = FALSE, message = FALSE}
...
LASSO model is similar to Ridge regression. The main difference is that the penalty term is replaced, which forces some *beta* coefficients to be zero. Therefore, while Ridge tends to include all variables in the final model because a higher *lamda* only reduces the beta coefficients towards zero. In sum, LASSO allows the selection/exclusion of variables since a significant tuning parameter should force several *beta* coefficients to be equal to zero.
